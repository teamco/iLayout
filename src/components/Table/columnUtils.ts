...
export function getColumnEffectiveKey<T extends object>(item: TColumn<T>): string {
...
export function getColumnLabel<T extends object>(item: TColumn<T>): string {
  ...
  if (title && typeof title === 'object' && 'props' in (title as Record<string, unknown>)) {
    const children = (title as Record<string, unknown> & { props: Record<string, unknown> }).props?.children;
    if (typeof children === 'string' || typeof children === 'number') return String(children);

    if (Array.isArray(children)) {
      const parts = children
        .filter((ch: unknown) => typeof ch === 'string' || typeof ch === 'number')
...
export function filterOutColumns<T extends object>(
  ...
  return columns.map((item) => ({
    ...item,
    hidden: item.concealable && !selectedColumns.includes(getColumnEffectiveKey(item as TColumn<T>) as string),
  })) as TColumns<T>;
}
